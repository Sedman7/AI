# подключаем модуль для работы с сым
import csv
...
def WordCheck(cWord):
    # выполняем лематизацию слова
    cWordNormal = morph.parse(cWord)[0].normal_form

    off = 0                         # по умолчанию результат = 0
    for i in range(len(wList)):     # начинаем поиск
        if wList[i][1] == cWordNormal:    # если нашли совпадение слова
