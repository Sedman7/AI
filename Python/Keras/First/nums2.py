...
canv_s = tk.Canvas(window, width=28, height=28, bg="white")                   # создали канву для рисования
canv_s.pack()
...
# вывод кропнутой картинки на верхний канвас, потом доделаю, если будет нужно
    # Создаем объект Image
    # ims = Image.new("L", (28, 28))
    # Заполняем пиксели изображения значениями из массива
    # ims.putdata([pixel for row in image for pixel in row])
    # Создаем объект ImageTk.PhotoImage
    # photo = ImageTk.PhotoImage(ims)
    # Отображаем изображение на холсте
    # canv_s.create_image(14, 14, image=photo)

    image = image / 255

    # zz = Image.fromarray(photo)
    # print(zz)
